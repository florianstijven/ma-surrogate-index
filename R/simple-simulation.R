...
meta_analytic_data_list = future_map(data_set_indicator, function(i) {
  generate_meta_analytic_data(N = 10,
                              n = 3e3,
                              train_clinical_prediction_model)
...
true_rho_surrogate = rho_delta_method(moment_estimates_MC$coefs, moment_estimates_MC$vcov)$rho
...
meta_analytic_data_simulated =
  bind_rows(
    tibble(data_set = 1:N_MC, treatment_effects = treatment_effects_list) %>% mutate(analysis_type = "Original surrogate", true_rho = true_rho_surrogate),
...
saveRDS(meta_analytic_data_simulated, "results/raw-results/simple-simulation/meta-analytic-data-simulated.rds")
